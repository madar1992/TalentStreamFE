...
function ResumeBuilder() {
  ...
  useEffect(() => {
    const apiUrl1 = 'https://43.204.125.6:5173/api/auth/login';
    ...
    if (requestData) {
      ...
      fetch(apiUrl1, requestOptions)
        ...
        .then(data => {
          const loginUrl = `https://43.204.125.6:5173/auth/login?identifier=${encodeURIComponent(requestData.identifier)}&password=${encodeURIComponent(requestData.password)}`;
